...
class TestGetDataAlpha(BaseVDERPCommandTestCase):

    def setUp(self):
        print('running setup')
        self.clear()
                
        self.lut_data['command']='GetAlpha'
        self.lut_data['fieldValue']='ABC'
    ...
    def testGetData(self):
        ...
        self.assertTrue(gdObj.priority_prompt, "Priority prompt should be false for GetAlpha command")
        
        ''' Verify that the constructor updated the legacy command to a GetData with correct dataType setting '''
        self.assertEqual(gdObj.datatype, 'Alpha', \
                         "Actual: '%s' Expected: '%s" % (gdObj.datatype, 'Alpha'))
    ...
    def testGetDataAlphaReadyDialog(self):
        ...
        self.post_dialog_responses('ABCVRESERVE001')
    ...
    def testGetDataAlphaValidateDialog(self):
        ...
        self.post_dialog_responses('PQR','ABC')
        ...
        self.validate_prompts('This is a prompt', 'Validate pre prompt <Spell>PQR</Spell>, validate post prompt')
    ...
    def testGetDataAlphaConfirmDialog(self):
        ...
        self.post_dialog_responses('ABCVRESERVE001','VRESERVE003')
        ...
        #check that the prompts were spoken correctly
        self.validate_prompts('This is a prompt', 'Confirm spoken value pre prompt <Spell>ABC</Spell>, Confirm spoken value post prompt')

    def testGetDataAlphaNumericMinGreaterMaxDialog(self):
        # queue up some responses to your prompts 
        ...
        try:
            ...
            if(len(mock_catalyst.log_messages) == 0):        
                ...
                objString = 'Min Length greater than max length for command GetAlpha'
                self.assertEqual(objString, mock_catalyst.log_messages[0])

            

    def testGetDataAlphaNumericNoReadyDialog(self):
        # queue up some responses to your prompts 
    ...
    def testGetDataAlphaNumericGreaterThanMaxDialog(self):
        # queue up some responses to your prompts 
        ''' the purpose of this test is to Check the if max is set then words spoken'''
        ''' greater than the max length are ignored''' 
        self.post_dialog_responses('ABCDEFG!')
        
        self.lut_data['ready']=0
        self.lut_data['fieldValue'] = 'ABCDEF'
        self.lut_data['minLength'] = 3
